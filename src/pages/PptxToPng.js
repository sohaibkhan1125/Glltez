...
import { convertPptxToPng, downloadFile } from '../services/mergePdfApi';
...
function PptxToPng() {
  ...
  const selectFile = (selected) => {
    const pptx = Array.from(selected).find((f) => {
    ...
    if (!pptx) {
      setError('Please select a valid .pptx or .ppt file.');
      return;
    }
  ...
  const handleConvert = async (e) => {
    ...
    try {
      const conversion = await convertPptxToPng(file, { pageRange: slideRange, resolution });
      setResult(conversion);
      setSuccess(
        conversion.type === 'zip'
          ? `${conversion.images.length} slides converted! Download the ZIP file below.`
          : 'PowerPoint converted to PNG successfully!'
      );
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-cream-50">
      <ToolNavbar toolName="PPTX to PNG" />
      ...
      <main className="section-container py-8 sm:py-12">
        <div className="mx-auto max-w-3xl">
          <div className="mb-8">
            ...
            <h1 className="mt-4 text-3xl font-extrabold text-stone-900 sm:text-4xl">PPTX to PNG</h1>
            <p className="mt-2 text-base text-stone-500">
              Convert PowerPoint slides to high-quality PNG images. Multi-slide decks are packaged into a ZIP file.
            </p>
          </div>
          ...
          <form onSubmit={handleConvert} className="rounded-2xl border border-stone-200 bg-white p-6 shadow-card">
            <div className="grid gap-5 sm:grid-cols-2">
              ...
              <div>
                <label htmlFor="slideRange" className="block text-sm font-bold text-stone-700">
                  Slide Range <span className="font-normal text-stone-400">(optional)</span>
                </label>
                <input
                  id="slideRange"
                  type="text"
            ...
            >
              ...
              <p className="mt-4 text-sm font-bold text-stone-700">Drag & drop a PowerPoint file here</p>
              <p className="mt-1 text-sm text-stone-500">.pptx or .ppt</p>
              <button
                type="button"
            ...
            {result?.images?.length > 1 && (
              <div className="mt-5 rounded-xl border border-stone-200 bg-stone-50 p-4">
                <p className="mb-3 text-sm font-bold text-stone-700">
                  Converted slides ({result.images.length})
                </p>
