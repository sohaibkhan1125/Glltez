...
import { convertPngToSvg, downloadFile } from '../services/mergePdfApi';
...
function PngToSvg() {
  ...
  const handleConvert = async (e) => {
    ...
    try {
      ...
      setSuccess('PNG converted to SVG successfully! Download your file below.');
    } catch (err) {
  ...
  const handleDownload = async () => {
    ...
    setError('');
    try {
      await downloadFile(downloadUrl, outputFileName || 'image.svg');
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-cream-50">
      <ToolNavbar toolName="PNG to SVG" />
      ...
      <main className="section-container py-8 sm:py-12">
        <div className="mx-auto max-w-3xl">
          <div className="mb-8">
            ...
            <h1 className="mt-4 text-3xl font-extrabold text-stone-900 sm:text-4xl">PNG to SVG</h1>
            <p className="mt-2 text-base text-stone-500">
              Convert PNG images to scalable SVG vector format for logos, icons, and web graphics.
            </p>
          </div>
              ...
              <div className="mt-5 rounded-xl border border-stone-200 bg-stone-50 px-4 py-4">
                ...
                {outputSize != null && (
                  <p className="mt-2 text-xs text-stone-500">
                    SVG size: <span className="font-bold text-orange-700">{formatFileSize(outputSize)}</span>
                  </p>
                )}
              </div>
            )}
