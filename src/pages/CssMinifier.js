...
function minifyCss(input) {
  ...
  preserved.forEach((block, index) => {
    css = css.replace(`__PRESERVE_${index}__`, block);
  });
...
function CssMinifier() {
  ...
  const handleMinify = () => {
    ...
    if (!trimmed) {
      setError('Please paste CSS to minify.');
      return;
    }

    if (!/[{}:;]/.test(trimmed)) {
      setError('Input does not look like CSS. Include rules with selectors and properties.');
      return;
    }

    try {
      const minified = minifyCss(trimmed);
      ...
      setSuccess(
        saved > 0
          ? `CSS minified successfully — ${percent}% smaller (${formatFileSize(saved)} saved).`
  ...
  return (
    <div className="min-h-screen bg-cream-50">
      <ToolNavbar toolName="CSS Minifier" />
      ...
      <main className="section-container py-8 sm:py-12">
        <div className="mx-auto max-w-5xl">
          <div className="mb-8">
            ...
            <h1 className="mt-4 text-3xl font-extrabold text-stone-900 sm:text-4xl">CSS Minifier</h1>
            <p className="mt-2 text-base text-stone-500">
              Remove comments and whitespace from CSS to reduce file size — runs entirely in your browser.
            </p>
          </div>
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            <div className="rounded-2xl border border-stone-200 bg-white p-5 shadow-card">
              <label htmlFor="css-input" className="block text-sm font-bold text-stone-700">
                Input CSS
              </label>
              <textarea
                id="css-input"
            ...
            <div className="rounded-2xl border border-stone-200 bg-white p-5 shadow-card">
              <div className="flex items-center justify-between">
                <label htmlFor="css-output" className="block text-sm font-bold text-stone-700">
                  Minified CSS
                </label>
                {output && (
                  <button
