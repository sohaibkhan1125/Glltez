...
import { convertPngToJpg, downloadFile } from '../services/mergePdfApi';
...
function PngToJpg() {
  ...
  const handleConvert = async (e) => {
    ...
    try {
      ...
      setSuccess('PNG converted to JPG successfully! Download your file below.');
    } catch (err) {
  ...
  const handleDownload = async () => {
    ...
    setError('');
    try {
      await downloadFile(downloadUrl, outputFileName || 'image.jpg');
    } catch (err) {
      setError(err.message || 'Failed to download JPG.');
    }
  };

  return (
    <div className="min-h-screen bg-cream-50">
      <ToolNavbar toolName="PNG to JPG" />
      ...
      <main className="section-container py-8 sm:py-12">
        <div className="mx-auto max-w-3xl">
          <div className="mb-8">
            ...
            <h1 className="mt-4 text-3xl font-extrabold text-stone-900 sm:text-4xl">PNG to JPG</h1>
            <p className="mt-2 text-base text-stone-500">
              Convert PNG images to JPG format for smaller file sizes and broad compatibility.
            </p>
          </div>
              ...
              <div className="mt-5 rounded-xl border border-stone-200 bg-stone-50 px-4 py-4">
                ...
                {outputSize != null && (
                  <p className="mt-2 text-xs text-stone-500">
                    JPG size: <span className="font-bold text-orange-700">{formatFileSize(outputSize)}</span>
                  </p>
                )}
              </div>
            )}
