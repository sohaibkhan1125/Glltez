...
import { convertPdfToDocx, downloadFile } from '../services/mergePdfApi';
...
function PdfToWord() {
  ...
  const handleConvert = async (e) => {
    ...
    try {
      ...
      setSuccess('PDF converted successfully! Download your Word document below.');
    } catch (err) {
  ...
  const handleDownload = async () => {
    ...
    setError('');
    try {
      await downloadFile(downloadUrl, outputFileName || 'document.docx');
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-cream-50">
      <ToolNavbar toolName="PDF to Word" />
      ...
      <main className="section-container py-8 sm:py-12">
        <div className="mx-auto max-w-3xl">
          <div className="mb-8">
            ...
            <h1 className="mt-4 text-3xl font-extrabold text-stone-900 sm:text-4xl">PDF to Word</h1>
            <p className="mt-2 text-base text-stone-500">
              Convert PDF documents to editable Word (.docx) files in seconds — no software required.
            </p>
          </div>
                ...
                {outputSize != null && (
                  <div className="mt-3 border-t border-stone-200 pt-3">
                    <p className="text-sm text-stone-600">
                      Word file size: <span className="font-bold text-sky-700">{formatFileSize(outputSize)}</span>
                    </p>
                  </div>
                )}
              </div>
