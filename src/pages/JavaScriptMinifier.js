...
function minifyJs(input) {
  ...
  js = js.replace(/`(?:\\.|[^`\\])*`/g, preserve);
  js = js.replace(/"(?:\\.|[^"\\])*"/g, preserve);
  js = js.replace(/'(?:\\.|[^'\\])*'/g, preserve);
  js = js.replace(/\/(?![*/])(?:\\.|[^/\\])+\/[dgimsuvy]*/g, preserve);
  js = js.replace(/\/\*[\s\S]*?\*\//g, '');
  js = js.replace(/(^|[^:])\/\/[^\n\r]*/gm, '$1');
  js = js.replace(/\s+/g, ' ');
  ...
  preserved.forEach((block, index) => {
    js = js.replace(`__PRESERVE_${index}__`, block);
  });
...
function JavaScriptMinifier() {
  ...
  const handleMinify = () => {
    ...
    if (!trimmed) {
      setError('Please paste JavaScript to minify.');
      return;
    }

    try {
      const minified = minifyJs(trimmed);

      try {
        // eslint-disable-next-line no-new-func
        new Function(minified);
      } catch {
        setError('Minified output may be invalid JavaScript. Check your input for edge cases (e.g. regex or ASI).');
        setOutput(minified);
        return;
      }
      ...
      setSuccess(
        saved > 0
          ? `JavaScript minified successfully — ${percent}% smaller (${formatFileSize(saved)} saved).`
          : 'JavaScript minified successfully.'
      );
    } catch (err) {
      setError(err.message || 'Failed to minify JavaScript.');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-cream-50">
      <ToolNavbar toolName="JavaScript Minifier" />
      ...
      <main className="section-container py-8 sm:py-12">
        <div className="mx-auto max-w-5xl">
          <div className="mb-8">
            ...
            <h1 className="mt-4 text-3xl font-extrabold text-stone-900 sm:text-4xl">JavaScript Minifier</h1>
            <p className="mt-2 text-base text-stone-500">
              Remove comments and whitespace from JavaScript to reduce file size — runs entirely in your browser.
            </p>
          </div>
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            <div className="rounded-2xl border border-stone-200 bg-white p-5 shadow-card">
              <label htmlFor="js-input" className="block text-sm font-bold text-stone-700">
                Input JavaScript
              </label>
              <textarea
                id="js-input"
            ...
            <div className="rounded-2xl border border-stone-200 bg-white p-5 shadow-card">
              <div className="flex items-center justify-between">
                <label htmlFor="js-output" className="block text-sm font-bold text-stone-700">
                  Minified JavaScript
                </label>
                {output && (
                  <button
