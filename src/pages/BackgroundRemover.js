...
import { downloadBlobUrl, removeImageBackground } from '../services/removeBgApi';

const SIZE_OPTIONS = [
  { value: 'auto', label: 'Auto — best fit for the image' },
  { value: 'regular', label: 'Regular — up to 25 megapixels' },
  { value: 'hd', label: 'HD — up to 4 megapixels' },
];
...
function BackgroundRemover() {
  ...
  const selectFile = (selected) => {
    const image = Array.from(selected).find((f) => {
  ...
  const handleRemoveBackground = async (e) => {
    ...
    try {
      const output = await removeImageBackground(file, { size });
      setResult(output);
      setSuccess('Background removed successfully! Download your transparent PNG below.');
    } catch (err) {
  ...
  const handleDownload = () => {
    ...
    setError('');
    try {
      downloadBlobUrl(result.url, result.fileName || 'no-bg.png');
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-cream-50">
      <ToolNavbar toolName="Background Remover" />
      ...
      <main className="section-container py-8 sm:py-12">
        <div className="mx-auto max-w-3xl">
          <div className="mb-8">
            ...
            <h1 className="mt-4 text-3xl font-extrabold text-stone-900 sm:text-4xl">Background Remover</h1>
            <p className="mt-2 text-base text-stone-500">
              Remove image backgrounds instantly with AI — upload a photo and get a transparent PNG.
            </p>
          </div>

          <form onSubmit={handleRemoveBackground} className="rounded-2xl border border-stone-200 bg-white p-6 shadow-card">
            <label htmlFor="size" className="block text-sm font-bold text-stone-700">
              Output Size
            </label>
            <select
              id="size"
            ...
            >
              ...
              <p className="mt-4 text-sm font-bold text-stone-700">Drag & drop an image here</p>
              <p className="mt-1 text-sm text-stone-500">JPG, PNG, or WebP</p>
              <button
                type="button"
              ...
              <div className="mt-5 rounded-xl border border-stone-200 bg-stone-50 px-4 py-4">
                ...
                {result?.fileSize != null && (
                  <p className="mt-2 text-xs text-stone-500">
                    Output size: <span className="font-bold text-orange-700">{formatFileSize(result.fileSize)}</span>
                  </p>
                )}
              </div>
            )}
            ...
            {(previewUrl || result?.url) && (
              <div className="mt-5 grid gap-4 sm:grid-cols-2">
                {previewUrl && (
                  <div>
                    <p className="mb-2 text-sm font-bold text-stone-700">Original</p>
                    <div className="overflow-hidden rounded-xl border border-stone-200 bg-white">
                      <img src={previewUrl} alt="Original upload" className="max-h-64 w-full object-contain" />
                    </div>
                  </div>
                )}
                {result?.url && (
                  <div>
                    <p className="mb-2 text-sm font-bold text-stone-700">Background removed</p>
                    <div
                    ...
                    >
                      <img src={result.url} alt="Background removed result" className="max-h-64 w-full object-contain" />
                    </div>
                  </div>
                )}
              </div>
